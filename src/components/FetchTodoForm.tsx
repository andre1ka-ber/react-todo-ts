...
const FetchTodoForm: React.FC = () => {
  ...
  const inputChangeHandler = (event: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>) => {
  ...
  return (
    <form className={"fetch-form"} onSubmit={submitHandler}>
      <div className={"input-field"}>
        ...
        <select name="completed" id="todo-status" className={"input-select"} value={fetchForm.completed} onChange={inputChangeHandler}>
      ...
      <div className={"input-field"}>
        ...
        <input name="count" id="todo-count" type={"number"} step={10} value={fetchForm.count} onChange={inputChangeHandler}/>
      </div>
      <button className={"p-4 rounded-md bg-yellow-500 text-white mb-4"} type="submit">Загрузить данные</button>
    </form>
  )
}
